...
def product(request):
    model = Product
    queryset = Product.objects.filter(zipcode = request.user.zipfield).order_by("-date_created")
    ...
    try:
        give = paginator.page(page)
    except PageNotAnInteger:
        # If page is not an integer, deliver first page.
        give = paginator.page(1)
    except EmptyPage:
        # If page is out of range (e.g. 9999), deliver last page of results.
        give = paginator.page(paginator.num_pages)

    if request.method == 'POST':
        form = ProductForm(request.POST, request.FILES)
        if form.is_valid():
            give= Product(user = request.user, title = request.POST['title'], docfile = request.FILES['docfile'], zipcode = request.POST['zipcode'], description = request.POST['description'], contact_method = request.POST['contact_method'], contact_info = request.POST['contact_info'])
            give.save()
...
def list1(request):
    if request.method == 'POST':
        form = ProductForm(request.POST, request.FILES)
        if form.is_valid():
            newdoc = Product(user = request.user, title = request.POST['title'], docfile = request.FILES['docfile'],zipcode = request.POST['zipcode'], description = request.POST['description'], contact_method = request.POST['contact_method'], contact_info = request.POST['contact_info'])
            newdoc.save()
